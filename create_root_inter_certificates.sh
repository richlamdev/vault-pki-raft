...
# if the VAULT_ROLE value do not match, certificates will not be signed/issued
...
vault write -address=$ADDRESS -tls-skip-verify \
      -field=certificate pki/root/generate/internal \
      common_name="$CN_ROOT" ttl=87600h | \
      tee "$ROOT_INTER_FOLDER/$CN_ROOT_NO_SPACE.root_cert.crt"
echo
...
vault write -address=$ADDRESS -tls-skip-verify \
      pki/config/urls issuing_certificates="https://127.0.0.1:8200/v1/pki/ca" \
...
vault secrets tune -address=$ADDRESS -tls-skip-verify \
      -max-lease-ttl=43800h pki_int
echo

# Generate an intermediate and save the CSR as $CN_pki_intermediate.csr
vault write -address=$ADDRESS -tls-skip-verify \
      -format=json pki_int/intermediate/generate/internal \
      common_name="$CN_INTER" | jq -r '.data.csr' > \
      "$ROOT_INTER_FOLDER/$CN_INTER_NO_SPACE"_pki_intermediate.csr
echo
...
vault write -address=$ADDRESS -tls-skip-verify \
      -format=json pki/root/sign-intermediate \
      csr=@"$ROOT_INTER_FOLDER/$CN_INTER_NO_SPACE"_pki_intermediate.csr \
      format=pem_bundle ttl="43800h" | jq -r '.data.certificate' > \
      "$ROOT_INTER_FOLDER/$CN_INTER_NO_SPACE"_signed_by_root.cert.pem
echo

# Import the signed CSR into Vault
vault write -address=$ADDRESS -tls-skip-verify pki_int/intermediate/set-signed \
      certificate=@"$ROOT_INTER_FOLDER/$CN_INTER_NO_SPACE"_signed_by_root.cert.pem
echo
...
vault write -address=$ADDRESS -tls-skip-verify pki_int/roles/"$VAULT_ROLE" \
      allowed_domains="$DOMAIN" allow_subdomains=true max_ttl="43800h"
echo

printf "\n%s" \
  "*** To view root certificate execute the following command:***"\
  "openssl x509 -in ${ROOT_INTER_FOLDER}/${CN_ROOT_NO_SPACE}.root_cert.crt\
  -text -noout"\
  ""\
  ""\
  "*** To view intermediate certificate execute the following command:***"\
  "openssl x509 -in ${ROOT_INTER_FOLDER}/${CN_INTER_NO_SPACE}_signed_by_root.cert.pem -text -noout"\
  ""\
  ""
