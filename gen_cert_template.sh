...
vault write -address=$ADDRESS -format=json pki_int/issue/"$ROLE" common_name=${COMMON_NAME} ip_sans="${IP_SAN1}" alt_names="${ALT_NAME1}" ttl=${TTL} | tee ${OUTPUT_FILE}
...
openssl rsa -in ${HOST}_cert.key -check | head -n20

echo
echo
echo
echo "*** View ${HOST}_cert.crt public certificate ***"
openssl x509 -in ${HOST}_cert.crt -text -noout | head -n50
